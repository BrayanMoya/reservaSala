     ...
     $(document).ready(function (){
		...
		*/
		...
		$('#ESTA_ID').change(function () {
		    table1
		        .columns( 9 )
		        .search( $('#ESTA_ID option:selected').text() )
		        .draw();
		} );
...
<table class="table table-striped" id="tabla">
	...
	<tbody>
		...
		<tr>
			...
			<td>{{ $reserva -> RESE_NOMPROGRAMA }}</td>
			<td>{{ $reserva -> RESE_CODGRUPO }}</td>
			<td>{{ $reserva -> RESE_CODMATERIA . ' - ' . $reserva -> RESE_NOMMATERIA }}</td>
			<td>{{ $reserva -> autorizaciones -> first() -> estado -> ESTA_DESCRIPCION }}</td>
			<td>{{ ($reserva -> RESE_USUARIOUNIAJC!=NULL) ? $reserva -> RESE_USUARIOUNIAJC :  $reserva -> RESE_CREADOPOR }}</td>
