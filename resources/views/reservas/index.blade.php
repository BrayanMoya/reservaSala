...
.borderesp {
	...
	background-color: rgb(153, 153, 102); z-index: 8;
	width:100px;
	border-radius: 8px;
}
...
<div class="panel panel-default">
	...
	@if(in_array(auth()->user()->rol->ROLE_ROL, ['admin', 'docente']))
	...
	<script type="text/javascript">
		...
		$(document).ready(function () {
			...
			$('#modalReserva').on('show.bs.modal', function (event) {
				...
				var userCurrentAcademusoft = "{{ session('useracademusoft') }}";
				var userCurrentAcademusoftTmp = userCurrentAcademusoft.replace('&ntilde;','ñ');
			//	console.log((decodeURIComponent(unescape(userCurrentAcademusoft))));
			//alert(userCurrentAcademusoftTmp);
				var rolCurrent = '{{ Auth::user()->rol->ROLE_ROL }}';

				if(userCurrentAcademusoftTmp == RESE_CREADOPOR || userCurrent == RESE_CREADOPOR || rolCurrent == 'admin'){
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-body">
				<h4>
					<em class="fa fa-cog fa-spin fa-3x fa-fw" style="vertical-align: middle;"></em> Cargando...
				</h4>
			</div>
		</div>
