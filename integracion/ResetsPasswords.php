...
trait ResetsPasswords
{
    ...
    public function sendResetLinkEmail(Request $request)
    {
        ...
        $msjparametro  = strtolower(getGlobalParameter('MENSAJE_USUARIO_INACTIVO_SMARTCAMPUS', 'Error'));
        $data = $this->sendResetLinkEmailWs($request->email);
        $data = strtolower($data[0]);

        if($msjparametro==$data){
            switch ($response) {
            ...
            $msjconfirmacion  = strtolower(getGlobalParameter('MENSAJE_RECOVERYPASSWORD_CONFIRMACION', 'OK'));
            return $this->getSendResetLinkEmailWs($msjconfirmacion);
        }

    }

    public function sendResetLinkEmailWs($user)
    {
        $soap = (new SoapController);
        @ $data = $soap->getRecuperarContrasenaWs($user);
        $this->getSendResetLinkEmailWs($data[0]);
        return $data;
    }

    protected function getSendResetLinkEmailWs($msj)
    {
